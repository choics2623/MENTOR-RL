...
from verl.utils.reward_score.mentor import remove_boxed, last_boxed_only_string
...
class MentorPipeline(BasicPipeline):
    def __init__(self, config, retriever=None, generator=None):
        ...
        self.prompt_template = prompt_template_dict['mentor_template_sys']
        ...
        self.mentor = Mentor(model_url, executor_url)
    ...
    def run_item(self, item):
        try:
            ...
            # 이 아이템만 스킵
            item.update_output("final_response", f"[ERROR in mentor.run] {type(e).__name__}: {e}")
            item.update_output("pred", "")
            item.update_output("skipped", True)
            return
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        with ThreadPoolExecutor(max_workers=30) as executor:
            future_to_item = {executor.submit(self.run_item, item): item for item in dataset}
            for future in tqdm(as_completed(future_to_item), total=len(future_to_item), desc="Inference: "):
                ...
                try:
                    ...
                    # 이 아이템만 스킵
                    item.update_output("final_response", f"[ERROR] {e}")
                    item.update_output("pred", "")
                    item.update_output("skipped", True)
                    continue
...
class MentorQwen3Pipeline(BasicPipeline):
    def __init__(self, config, retriever=None, generator=None):
        super().__init__(config)
        if retriever is None:
            retriever = get_retriever(config)
        ...
        # Use Qwen3 tokenizer for native chat template with tools
        self.tokenizer = AutoTokenizer.from_pretrained(config["generator_model_path"])

        model_url = config["sgl_remote_url"]
        executor_url = config["sandbox_url"]

        self.mentor = Mentor(model_url, executor_url, model_name='qwen3')
        self.save_dir = config["save_dir"]

        # Define Wikipedia search tool for Qwen3 native format
    ...
    def extract_last_assistant_response(self, text: str) -> str:
        ...
        # Remove content between <think> tags for Qwen3
        think_pattern = r'<think>.*?</think>'
        cleaned_response = re.sub(think_pattern, '', last_response, flags=re.DOTALL)
    ...
    def run_item(self, item):
        try:
            # Reset tool calls history for this sample
            self.mentor.reset_tool_calls_history()

            # For Qwen3, prepare question with boxed format instruction
            formatted_question = f"Question: {item.question}\n\nIf you have got the answer, enclose it within \\boxed{{}} with latex format."

            # Use Qwen3 native chat template with tools
            ...
            # Run with Mentor using env and tools for Qwen3
            response = self.mentor.run_qwen3_native(
                prompt=prompt,
                env=wikipedia_search_env.replace('<search-url-placeholder>', self.config["remote_retriever_url"]),
                tools=self.wikipedia_tools
            )
            ...
            tool_calls_info = {
                'tool_calls_count': self.mentor.tool_calls_count,
                'successful_tool_calls': self.mentor.successful_tool_calls,
                'failed_tool_calls': self.mentor.failed_tool_calls,
                'tool_calls_history': self.mentor.tool_calls_history
            }

            # Check if response is a retry failure dict or a successful string
            if isinstance(response, dict) and response.get("_retry_failed"):
                pred = f"[ERROR] {response.get('error', 'Unknown error')}"
            else:
                # response is a string containing the full conversation
                pred = remove_boxed(last_boxed_only_string(response))
                if not pred:
                    pred = self.extract_last_assistant_response(response)
        ...
        item.update_output("pred", pred)
        item.update_output("tool_calls_info", tool_calls_info)
