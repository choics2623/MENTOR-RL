...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_mentor = config.get('use_mentor', True)
        self.prompt_template_name = config.get('prompt_template_name', 'mentor_template_sys')
    ...
    def _pack_mentor_input(self, prompt_template, func_schemas, user_input):
        # Qwen3 native tool calling support
        if self.prompt_template_name == 'qwen3_native':
            # Parse func_schemas if it's a string
            import json
            if isinstance(func_schemas, str):
                try:
                    tools = json.loads(func_schemas)
                except:
                    tools = []
            else:
                tools = func_schemas if func_schemas else []
            ...
            return self.tokenizer.apply_chat_template(
                ...
                tokenize=False
            )  # No need to add <think> manually, template handles it
        else:
            # Legacy Qwen2.5 style
            return self.tokenizer.apply_chat_template([
    ...
    def _read_files_and_tokenize(self):
        ...
        if self.filter_overlong_prompts:
            ...
            if self.use_mentor:
                import json
                def get_func_schemas(doc):
                    try:
                        if isinstance(doc['extra_info'], str):
                            extra_info = json.loads(doc['extra_info'])
                        else:
                            extra_info = doc['extra_info']
                        return extra_info.get('func_schemas', '[]')
                    except:
                        return '[]'
                
                self.dataframe = self.dataframe.filter(
                    lambda doc: len(tokenizer.encode(self._pack_mentor_input(self.prompt_template, get_func_schemas(doc), doc[prompt_key]))
    ...
    def __getitem__(self, item):
        ...
        if self.use_mentor:
            ...
            try:
                if isinstance(row_dict['extra_info'], str):
                    extra_info = json.loads(row_dict['extra_info'])
                else:
                    extra_info = row_dict['extra_info']
                func_schemas = extra_info.get('func_schemas', '[]')
            except:
                func_schemas = '[]'
            prompt_with_chat_template = self._pack_mentor_input(self.prompt_template, func_schemas, chat)
        else:
        ...
        # Parse reward_model if it's a JSON string
        import json
        try:
            if isinstance(row_dict['reward_model'], str):
                row_dict['reward_model'] = json.loads(row_dict['reward_model'])
        except:
            row_dict['reward_model'] = {'ground_truth': [], 'style': 'rule'}
        ...
        if self.use_mentor:
            ...
            if row_dict['data_source'] == 'musique_mentor':
                assert self.config.get('search_url', None) is not None, "search_url for musique_mentor dataset is not set"
                row_dict['env'] = env.replace('<search-url-placeholder>', self.config.get('search_url', ''))
            else:
                row_dict['env'] = env
