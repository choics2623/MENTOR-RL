"""
ABLATION2 Reward Function - Tool Partial Matching (No Hallucination Check)
================================================================================
For datasets with tool usage only (teacher always uses tools):
- Base: 0.7 for correctness + 0.3 for partial tool matching
- Max total: 1.0 (0.7 + 0.3)
- Partial matching: Student gets bonus if ANY tool overlaps with teacher's tools
"""
...
def compute_score_with_format(tokenizer, solution_str, ground_truth, teacher_tools=None) -> tuple[float, str]:
    ...
    if not solution_str.endswith(tokenizer.eos_token):
        return 0.0, '[ABLATION2] Missing EOS token'

    # 2. Extract boxed answer
    try:
        boxed_string = last_boxed_only_string(solution_str)
        if not boxed_string:
            return 0.1, '[ABLATION2] Missing boxed answer but valid format'

        answer = remove_boxed(boxed_string)
        if not answer.strip():
            return 0.1, '[ABLATION2] Empty boxed answer but valid format'

    except Exception as e:
        return 0.1, f'[ABLATION2] Answer extraction error: {e}'
    ...
    # ABLATION2 Logic: Tool partial matching (any overlap)
    tool_matching = 0.0

    # Teacher should always have tools in this dataset
    if not teacher_used_tools:
        return 0.1, '[ABLATION2] Error: Teacher tools not provided'

    # Check if student used tools
    if student_used_tools:
        # ABLATION2: Partial tool matching (any overlap)
        overlap = teacher_tool_set & student_tool_set
        if len(overlap) > 0:
            tool_matching = 0.3
            bonus_reason = f"Partial match (+0.3) (T:{sorted(teacher_tool_set)}, S:{sorted(student_tool_set)}, Overlap:{sorted(overlap)})"
        else:
            bonus_reason = f"No overlap (+0.0) (T:{sorted(teacher_tool_set)}, S:{sorted(student_tool_set)})"
    else:
        bonus_reason = f"Student didn't use tools (+0.0) (T:{sorted(teacher_tool_set)}, S:[])"

    # Scale down tool bonus for wrong answers
    if is_correct:
        final_reward = correctness_reward + tool_matching
        print(f'[ABLATION2] Correct answer, Correctness={correctness_reward:.1f}, {bonus_reason}')
        return final_reward, f'[ABLATION2] Correct answer, Correctness={correctness_reward:.1f}, {bonus_reason}'
    else:
        # Wrong answer: very small bonus for tool matching (0.06 max)
        tool_bonus_scaled = tool_matching * 0.2  # 0.3 * 0.2 = 0.06 max
        final_reward = correctness_reward + tool_bonus_scaled
        bonus_reason_scaled = bonus_reason.replace('+0.3', f'+{tool_matching * 0.2:.1f}')
        print(f'[ABLATION2] Wrong answer, Correctness={correctness_reward:.1f}, {bonus_reason_scaled} (scaled down)')
        return final_reward, f'[ABLATION2] Wrong answer, Correctness={correctness_reward:.1f}, {bonus_reason_scaled} (scaled down)'
