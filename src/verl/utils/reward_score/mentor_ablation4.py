"""
ABLATION4 Reward Function - Tool F1 Matching Only
======================================================
For datasets with tool usage only (teacher always uses tools):
- Base: 0.7 for correctness + 0.3 * F1 for tool matching + 0.1 for no hallucination
- Max total: 1.1 (0.7 + 0.3 + 0.1)
- Focuses on correctness with F1-based tool matching bonus
"""
...
def mv_parse_equal(pred: str, gold: Union[str, List[str]]) -> bool:
    """Check if prediction equals ground truth using Math-Verify parse/verify"""
    golds = [gold] if isinstance(gold, str) else gold
    try:
...
def calculate_tool_f1(teacher_tools: List[str], student_tools: List[str]) -> float:
    """Calculate F1 score between teacher and student tool usage"""
    teacher_set = set(teacher_tools)
    student_set = set(student_tools)
    
    if len(teacher_set) == 0 and len(student_set) == 0:
        return 1.0  # Both empty = perfect match
    
    if len(teacher_set) == 0 or len(student_set) == 0:
        return 0.0  # One empty, one not = no match
    
    intersection = teacher_set & student_set
    
    if len(intersection) == 0:
        return 0.0  # No common tools
    
    precision = len(intersection) / len(student_set)
    recall = len(intersection) / len(teacher_set)
...
def compute_score_with_format(tokenizer, solution_str, ground_truth, teacher_tools=None) -> tuple[float, str]:
    ...
    if not solution_str.endswith(tokenizer.eos_token):
        return 0.0, '[ABLATION4] Missing EOS token'
    
    # 2. Extract boxed answer
    try:
        boxed_string = last_boxed_only_string(solution_str)
        if not boxed_string:
            return 0.1, '[ABLATION4] Missing boxed answer but valid format'
        
        answer = remove_boxed(boxed_string)
        if not answer.strip():
            return 0.1, '[ABLATION4] Empty boxed answer but valid format'
            
    except Exception as e:
        return 0.1, f'[ABLATION4] Answer extraction error: {e}'
    ...
    # ABLATION4 Logic: Tool F1 matching only (teacher always uses tools)
    tool_matching = 0.0

    # Teacher should always have tools in this dataset
    if not teacher_used_tools:
        return 0.1, '[ABLATION4] Error: Teacher tools not provided'

    # Calculate F1 between teacher and student tools
    if student_used_tools:
        tool_f1 = calculate_tool_f1(teacher_tools, student_tool_info['used_tools'])
        tool_matching = 0.3 * tool_f1

        teacher_set = set(teacher_tools)
        student_set = set(student_tool_info['used_tools'])

        if tool_f1 == 1.0:
            bonus_reason = f"Perfect match (+{tool_matching:.3f}) (T:{sorted(teacher_set)}, S:{sorted(student_set)})"
        elif tool_f1 > 0:
            bonus_reason = f"Partial match (+{tool_matching:.3f}, F1={tool_f1:.3f}) (T:{sorted(teacher_set)}, S:{sorted(student_set)})"
        else:
            bonus_reason = f"No match (+{tool_matching:.3f}) (T:{sorted(teacher_set)}, S:{sorted(student_set)})"
    else:
        tool_f1 = 0.0
        tool_matching = 0.0
        bonus_reason = f"Student didn't use tools (+0.0) (T:{sorted(set(teacher_tools))}, S:[])"
    ...
    if is_correct:
        ...
        print(f'[ABLATION4] Correct answer, Correctness={correctness_reward:.1f}, {bonus_reason}')
        return final_reward, f'[ABLATION4] Correct answer, Correctness={correctness_reward:.1f}, {bonus_reason}'
    else:
        ...
        # Update bonus reason with scaled values
        if 'Perfect match' in bonus_reason:
            bonus_reason_scaled = bonus_reason.replace(f'(+{tool_matching:.3f})', f'(+{tool_matching * 0.2:.3f})')
        elif 'Partial match' in bonus_reason:
            bonus_reason_scaled = bonus_reason.replace(f'(+{tool_matching:.3f}', f'(+{tool_matching * 0.2:.3f}')
        else:
            bonus_reason_scaled = bonus_reason
        print(f'[ABLATION4] Wrong answer, Correctness={correctness_reward:.1f}, {bonus_reason_scaled} (scaled down)')
        return final_reward, f'[ABLATION4] Wrong answer, Correctness={correctness_reward:.1f}, {bonus_reason_scaled} (scaled down)'